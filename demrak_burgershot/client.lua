...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.burgercook) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at lave burgere'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
								...
								local finished = exports['ac_progressbar']:startUI(10000, "Laver burgere...")
								Citizen.Wait(10000)
								ClearPedTasksImmediately(PlayerPedId())
								TriggerServerEvent('demrak_burgershot:addburger', 'item')
					end
				end
		end
	end
				...
				if not inMission then
					...
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						...
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
								...
								Citizen.Wait(5000)
								ClearPedTasksImmediately(PlayerPedId())
								TriggerServerEvent('demrak_burgershot:addcola', 'item')
					end
				end
		end
	end
...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.burgermake) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at pakke en burger'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "mini@repair" ) 
						TaskPlayAnim( PlayerPedId(), "mini@repair", "fixing_a_ped", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						local finished = exports['ac_progressbar']:startUI(9700, "Pakker en burger...")
						Citizen.Wait(9700)
						ClearPedTasks(PlayerPedId())
						TriggerServerEvent('demrak_burgershot:remove', 'item', 1, Config.Item)
						TriggerServerEvent('demrak_burgershot:add', 'item', 1, Config.Package)
					end
				end
		end
	end
...
Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.burgerpackage) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at sælge en burger'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.3) then
						ClearPedSecondaryTask(PlayerPedId())
						...
						local finished = exports['ac_progressbar']:startUI(2700, "Sælger en burger...")
						Citizen.Wait(2700)
						ClearPedTasks(PlayerPedId())
						TriggerServerEvent("demrak_burgershot:sellburger")
					end
				end
		end
	end
...
--[[Citizen.CreateThread(function()
	while true do
	Citizen.Wait(0)
		for k,v in pairs(Config.burgerDelivery) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at købe en burger'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						TriggerServerEvent("demrak_burgershot:buyburger")
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "anim@heists@keycard@" ) 
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						Citizen.Wait(850)
	...
	Citizen.Wait(0)
		for k,v in pairs(Config.JobPoints) do
			local coords = GetEntityCoords(GetPlayerPed(-1))
				if (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 3.0)  then
				if not inMission then
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at starte levering af burgere'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						TriggerServerEvent("demrak_burgershot:server")
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "anim@heists@keycard@" ) 
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
						Citizen.Wait(850)
					...
					DrawText3D(v.x, v.y, v.z, ('Tryk ~c~[~b~E~c~] ~w~for at anullere levering af burgere'))	
					if IsControlJustReleased(0, 46) and (GetDistanceBetweenCoords(coords, v.x, v.y, v.z, true) < 0.5) then
						ClearPedSecondaryTask(PlayerPedId())
						loadAnimDict( "anim@heists@keycard@" ) 
						TaskPlayAnim( PlayerPedId(), "anim@heists@keycard@", "exit", 8.0, 1.0, -1, 16, 0, 0, 0, 0 )
	...
	local d = GetDistanceBetweenCoords(pcoords, coords[1], coords[2], coords[3], true)
		if inMission == true and MissionCoords ~= false and d < 20.0 then
			...
			if IsControlJustReleased(0, 38) and d < 5.0 then
				...
				ClearPedTasks(PlayerPedId())
				TriggerServerEvent('demrak_burgershot:delivery')
			end
		end
	end
end)
